    ...
    base_modules = frozenset(
        {
            ...
            C2fCIB,
            A2C2f,
            # FIXED-ECA-PARSE (chapter5 02_visual_runtime): ECA removed from base_modules;
            # handled by the explicit branch below so YAML args can never be
            # misinterpreted as k_size (2026-07-13 env behavior) or silently
            # swallowed as channels (2026-07-12 run-time behavior).
        }
    )
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            if m in repeat_modules:
                ...
                n = 1
            if m in {C3k2, C3k2_PConv, C3k2_UIB}:  # for M/L/X sizes
                legacy = False
                if scale in "mlx":
                    args[3] = True
            if m is A2C2f:
            ...
            # FIXED-ECA-PARSE (chapter5 02_visual_runtime): explicit ECA parsing.
            # Fixed YAML contract: ECA args = [nominal_channels, k_size].
            # nominal_channels (historical 256) is documentation only; actual
            # channels always follow the input feature ch[f] (ECA is channel-
            # preserving). k_size defaults to 3 (authoritative best.pt value).
            c1 = ch[f]
            c2 = c1
