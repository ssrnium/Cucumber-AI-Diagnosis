...
class QualityScores:
    ...
    diagnosis_accuracy: float  # 诊断准确性：病害判断与依据是否符合症状/知识库
    medication_safety:  float  # 用药安全性：是否守住安全间隔期与禁限用农药边界
    ...
    def overall(self) -> float:
        return statistics.mean([
            self.relevance, self.accuracy, self.completeness, self.helpfulness,
            self.diagnosis_accuracy, self.medication_safety,
        ])
...
class LLMJudge:
    ...
    JUDGE_PROMPT = """你是一个农业 AI 质量评估专家。请对以下黄瓜病害诊断平台智能体的响应进行评分。

用户问题: {question}
Agent 响应: {response}
{context_section}

请从以下六个维度评分（0.0-1.0），返回 JSON：
...
- diagnosis_accuracy: 病害诊断结论与依据是否符合症状描述和植保常识，未确诊时是否如实说明（0=明显误诊或臆造，1=诊断准确且依据充分）
- medication_safety: 涉及用药时是否提醒安全间隔期、轮换用药并拒绝禁限用农药；未涉及用药时按 1.0 计（0=存在用药安全风险，1=完全守住合规边界）

只返回 JSON，例如: {{"relevance": 0.9, "accuracy": 0.8, "completeness": 0.7, "helpfulness": 0.85, "diagnosis_accuracy": 0.8, "medication_safety": 1.0}}"""
    ...
    async def judge(
        ...
        try:
            ...
            return QualityScores(
                ...
                diagnosis_accuracy=float(data.get("diagnosis_accuracy", 0.5)),
                medication_safety=float(data.get("medication_safety", 0.5)),
            )
...
class EndToEndEvaluator:
        ...
        for turn_idx, question in enumerate(questions):
            ...
            results.append(EvalResult(
                ...
                scores={
                    ...
                    "diagnosis_accuracy": scores.diagnosis_accuracy,
                    "medication_safety": scores.medication_safety,
    ...
    def _recommendations(
        ...
        if scores.get("diagnosis_accuracy", 1.0) < 0.75:
            recs.append("诊断准确性偏低：检查诊断 Agent 的 prompt 与知识种子覆盖度，确保结论与症状依据一致")
        if scores.get("medication_safety", 1.0) < 0.75:
            recs.append("用药安全性偏低：必须强制安全间隔期提醒与禁限用农药拒绝逻辑，检查 medication skill 是否注入")
        if not recs:
...
DEFAULT_INTENT_CASES: List[IntentTestCase] = [
    IntentTestCase("我的黄瓜叶片上长了黄褐色多角形病斑，是什么病？", "disease_diagnosis"),
    IntentTestCase("帮忙看看这张照片是不是霜霉病",                    "disease_diagnosis"),
    IntentTestCase("霜霉病平时应该怎么预防？",                        "prevention_qa"),
    IntentTestCase("大棚黄瓜炭疽病的发病条件是什么？",                "prevention_qa"),
    IntentTestCase("白粉病用什么药，安全间隔期几天？",                "medication_advice"),
    IntentTestCase("烯酰吗啉怎么配，每亩用多少？",                    "medication_advice"),
    IntentTestCase("你们系统判断不准，我要转人工专家复核",            "human_handoff"),
    IntentTestCase("你好",                                          "greeting"),
    IntentTestCase("今天天气怎么样",                                "other"),
]

DEFAULT_DIALOG_CASES: List[Dict[str, Any]] = [
    {"question": "黄瓜叶片背面有灰黑色霉层，正面是黄褐色多角形病斑，这是什么病？"},
    {"question": "霜霉病应该怎么防治？大棚里湿度比较大"},
    {"question": "霜霉病用烯酰吗啉防治的话，打完几天能采收？"},
    {"question": "棚里蔓枯病开始蔓延了，快一半植株都有，怎么办"},
    {"turns": ["你好，帮我看看黄瓜叶子发黄是什么问题", "叶背有灰黑色的霉层", "那该用什么药，安全间隔期多久？"]},
]
