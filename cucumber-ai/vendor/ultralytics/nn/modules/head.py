...
class TCDetect(Detect):
    """Text-Contrastive Detection head (chapter5 TCDH experiment, prereg file 19).

    Classification logits are cosine similarities between a 512-d visual region
    embedding and frozen text prototypes: scores = cos(e, T) * exp(logit_scale) + bias.

    Only the final Conv2d(c3->nc) of each cv3 tower is replaced by Conv2d(c3->embed);
    box branch (cv2/DFL), backbone and neck are untouched. The similarity logits
    feed v8DetectionLoss / TaskAlignedAssigner directly. forward keeps the
    single-argument signature (prototypes live in a buffer, not passed in).

    Arms (prereg): S1 = real prototypes frozen; S2 = random orthogonal anchors
    frozen (pre-generated, shared across seeds); S3 = real prototypes, unfrozen.
    """

    def __init__(self, nc: int = 80, reg_max=16, end2end=False, ch: tuple = (), embed: int = 512):
        """Initialize TCDetect: keep cv3 towers, swap final conv to embed-dim projection."""
        super().__init__(nc, reg_max, end2end, ch)
        c3 = max(ch[0], min(self.nc, 100))  # single scalar (64 for ch=(64,128,256), nc=5)
        for i in range(self.nl):
            self.cv3[i][-1] = nn.Conv2d(c3, embed, 1)  # 512-d region embedding output
        self.embed = embed
        # Orthogonal init so F.normalize is well-defined during the stride-building
        # forward in DetectionModel.__init__ (runs before set_text()).
        proto = torch.empty(nc, embed)
        nn.init.orthogonal_(proto)
        self.register_buffer("text_prototypes", proto, persistent=True)
        self.logit_scale = nn.Parameter(torch.ones([]) * torch.tensor(1 / 0.07).log())
        self.bias = nn.Parameter(torch.tensor([-10.0]))

    def set_text(self, t: torch.Tensor, trainable: bool = False):
        """Inject text prototypes. trainable=True converts them to a Parameter (S3 arm)."""
        assert t.shape == (self.nc, self.embed), f"text shape {tuple(t.shape)} != {(self.nc, self.embed)}"
        t = t.detach().float().clone()
        if trainable:
            self.text_prototypes = nn.Parameter(t)  # buffer -> Parameter replacement
        else:
            with torch.no_grad():
                self.text_prototypes.copy_(t)

    def forward_head(self, x: list[torch.Tensor], box_head=None, cls_head=None) -> dict:
        """Compute boxes via the stock box branch and scores via text-similarity logits."""
        if box_head is None or cls_head is None:  # for fused inference
            return dict()
        bs = x[0].shape[0]
        boxes = torch.cat([box_head[i](x[i]).view(bs, 4 * self.reg_max, -1) for i in range(self.nl)], dim=-1)
        tn = F.normalize(self.text_prototypes, dim=-1)  # [nc, embed]
        scale = self.logit_scale.exp()
        scores = []
        for i in range(self.nl):
            e = F.normalize(cls_head[i](x[i]), dim=1)  # [bs, embed, h, w]
            s = torch.einsum("bdhw,kd->bkhw", e, tn) * scale + self.bias  # [bs, nc, h, w]
            scores.append(s.view(bs, self.nc, -1))
        return dict(boxes=boxes, scores=torch.cat(scores, dim=-1), feats=x)

    def bias_init(self):
        """Only the box branch gets the stride prior; the cls path is cosine similarity
        with its own logit_scale/bias, so the stock Detect.bias_init (which writes the
        cls conv bias) must not run."""
        for a in self.one2many["box_head"]:
            a[-1].bias.data[:] = 2.0
        if self.end2end:
            for a in self.one2one["box_head"]:
                a[-1].bias.data[:] = 2.0
