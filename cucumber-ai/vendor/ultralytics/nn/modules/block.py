...
class PConv(nn.Module):
    """Partial convolution from FasterNet using the training-safe split-cat path.

    Only ``1 / n_div`` of the input channels are processed by a regular 3x3
    convolution. The remaining channels bypass the spatial convolution and are
    concatenated back unchanged. Input and output channel counts are identical.
    """
    ...
    def __init__(self, dim: int, n_div: int = 4):
        ...
        if not isinstance(n_div, int) or n_div <= 1:
            raise ValueError(f"n_div must be an integer greater than 1, got {n_div!r}")
        self.dim_conv3 = dim // n_div
        self.dim_untouched = dim - self.dim_conv3
        if self.dim_conv3 < 1:
            raise ValueError(f"dim={dim} is too small for n_div={n_div}")
...
class Bottleneck_PConv(nn.Module):
    """YOLO bottleneck: 1x1 Conv -> PConv -> 1x1 Conv, with optional residual."""
    ...
    def __init__(
        ...
        super().__init__()
        if g != 1:
            raise ValueError("Bottleneck_PConv currently supports g=1 only; PConv is not grouped convolution.")
        c_ = int(c2 * e)
        if c_ < 1:
...
class _C3k_PConv(C3):
    """Private C3k analogue whose internal bottlenecks use PConv."""
    ...
    def __init__(
        ...
        self.m = nn.Sequential(
            *(Bottleneck_PConv(c_, c_, shortcut, g, e=1.0, n_div=n_div) for _ in range(n))
        )


class C3k2_PConv(C2f):
    """Ultralytics 8.4.90 C3k2 adapted with PConv bottlenecks."""
    ...
    def __init__(
        ...
        self.m = nn.ModuleList(
            nn.Sequential(
                Bottleneck_PConv(self.c, self.c, shortcut, g, e=1.0, n_div=n_div),
                PSABlock(self.c, attn_ratio=0.5, num_heads=max(self.c // 64, 1)),
            )
            if attn
            else _C3k_PConv(self.c, self.c, 2, shortcut, g, e=0.5, n_div=n_div)
            if c3k
...
class UIBBlock(nn.Module):
    """PyTorch adaptation of MobileNetV4 Universal Inverted Bottleneck (UIB).

    The block supports the four UIB instantiations through optional depthwise
    convolutions:

    - IB:        start_dw=0, middle_dw>0
    - ConvNeXt:  start_dw>0, middle_dw=0
    - FFN:       start_dw=0, middle_dw=0
    - ExtraDW:   start_dw>0, middle_dw>0

    The starting and ending depthwise convolutions are linear (no activation),
    while the middle depthwise convolution uses the normal YOLO activation.
    This follows the operation ordering described by the official TensorFlow
    MobileNetV4 implementation, while retaining Ultralytics Conv/BN/SiLU
    conventions for integration with YOLO11.

    Notes:
        This is a YOLO11-oriented PyTorch adaptation, not an official Google
        PyTorch implementation. Stochastic depth and dilation are intentionally
        omitted from the first-round detector adaptation.
    """
    ...
    def __init__(
        ...
        if not isinstance(c1, int) or c1 <= 0:
            raise ValueError(f"c1 must be a positive integer, got {c1!r}")
        if not isinstance(c2, int) or c2 <= 0:
            raise ValueError(f"c2 must be a positive integer, got {c2!r}")
        if stride not in (1, 2):
            raise ValueError(f"stride must be 1 or 2, got {stride!r}")
        if not isinstance(expand_ratio, (int, float)) or expand_ratio <= 0:
            raise ValueError(f"expand_ratio must be positive, got {expand_ratio!r}")
        ...
        for name, kernel in (
            ...
            if not isinstance(kernel, int) or kernel < 0 or (kernel > 0 and kernel % 2 == 0):
                raise ValueError(f"{name} must be 0 or a positive odd integer, got {kernel!r}")

        if stride > 1:
            if middle_dw_downsample and middle_dw_kernel_size == 0:
                raise ValueError("Downsampling requested at a missing middle depthwise convolution.")
            if not middle_dw_downsample and start_dw_kernel_size == 0:
                raise ValueError("Downsampling requested at a missing starting depthwise convolution.")

        hidden = max(1, int(round(c1 * float(expand_ratio))))
        self.use_residual = bool(shortcut and stride == 1 and c1 == c2)

        start_stride = stride if (stride > 1 and not middle_dw_downsample) else 1
        middle_stride = stride if (stride > 1 and middle_dw_downsample) else 1

        self.start_dw = (
            Conv(c1, c1, k=start_dw_kernel_size, s=start_stride, g=c1, act=False)
            if start_dw_kernel_size
            else nn.Identity()
        )
        self.expand = Conv(c1, hidden, k=1, s=1)
        self.middle_dw = (
            Conv(hidden, hidden, k=middle_dw_kernel_size, s=middle_stride, g=hidden)
            if middle_dw_kernel_size
            else nn.Identity()
        )
        self.project = Conv(hidden, c2, k=1, s=1, act=False)
        self.end_dw = (
            Conv(c2, c2, k=end_dw_kernel_size, s=1, g=c2, act=False)
            if end_dw_kernel_size
            else nn.Identity()
        )

        if use_layer_scale:
            self.layer_scale = nn.Parameter(torch.full((1, c2, 1, 1), float(layer_scale_init_value)))
        else:
...
class Bottleneck_UIB(nn.Module):
    """YOLO bottleneck wrapper around UIBBlock with optional residual."""
    ...
    def __init__(
        ...
        super().__init__()
        if g != 1:
            raise ValueError("Bottleneck_UIB currently supports g=1 only.")
        c_ = int(c2 * e)
        if c_ < 1:
            raise ValueError(f"Hidden channels must be positive, got c2={c2}, e={e}")

        self.pre = Conv(c1, c_, 1, 1) if c1 != c_ else nn.Identity()
        self.block = UIBBlock(
            c_,
            c2,
            stride=1,
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        y = self.block(self.pre(x))
        return x + y if self.add else y


class _C3k_UIB(C3):
    """Private C3k analogue whose internal bottlenecks use UIB blocks."""
...
class C3k2_UIB(C2f):
    """Ultralytics 8.4.90 C3k2 adapted with fixed/configurable UIB blocks.

    The first eight parameters preserve the local C3k2 interface exactly.
    UIB-specific parameters are appended to avoid YAML argument displacement.
    """
    ...
    def __init__(
        ...
        def make_uib(channels: int) -> Bottleneck_UIB:
            return Bottleneck_UIB(
                channels,
                channels,
                shortcut=shortcut,
                g=g,
        ...
        self.m = nn.ModuleList(
            ...
            if c3k
            else make_uib(self.c)
            for _ in range(n)
        )


class ECA(nn.Module):
    """Efficient Channel Attention (ECA-Net) module.

    Proposed in ECA-Net: Efficient Channel Attention for Deep Convolutional Neural Networks
    (https://arxiv.org/abs/1910.03151).

    This implementation keeps c1/c2 arguments for compatibility with Ultralytics YAML parsing.
    ECA does not change spatial or channel dimensions, so c2 is ignored.
    """

    def __init__(self, c1: int, c2: int | None = None, k_size: int = 3):
        """Initialize ECA with adaptive average pooling and 1D convolution.

        Args:
            c1 (int): Number of input channels (kept for YAML compatibility).
            c2 (int | None): Number of output channels (unused, kept for compatibility).
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        y = self.avg_pool(x)
        # squeeze spatial dims, transpose for 1D conv: (B, 1, C)
