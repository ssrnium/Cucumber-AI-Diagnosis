...
16. 面试官视角强化（2026-09-18）：**cucumber-admin 单元测试 18 项**（诊断去重/权限边界/文件类型/AI 宕机/反馈流转/低置信复核/模型激活互斥/日志查询，mvn test 全绿）；**操作日志前端查询页**（系统管理→操作日志，admin 权限，菜单按权过滤）；来源弹窗加载骨架屏（消除"未查询到"闪现）。
